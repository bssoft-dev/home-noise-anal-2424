...
class MobileNetV2_MFCC(nn.Module):
    def __init__(self, num_classes=17, width_mult=1.):
...
def mobilenet_v2_mfcc(num_classes=17, pretrained=False, **kwargs):
    model = MobileNetV2_MFCC(num_classes=num_classes, **kwargs)
    if pretrained:
...
# 사용 예시
# model = mobilenet_v2_mfcc(num_classes=17)
# input_tensor = torch.randn(1, 1, 40, 100)  # (batch_size, channels, num_mfcc, time_steps)
